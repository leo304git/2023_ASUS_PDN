...
void OctAStarRouter::backTrace(int tXId, int tYId) {
    ...
    // (middle part)
    for (int xId = _tPos.first - floor(_exactWidth*wRatio); xId <= _tPos.first + floor(_exactWidth*wRatio); ++ xId) {
    ...
    for (int d = 1; d <= floor(_exactWidth/2.0)-floor(_exactWidth*wRatio); ++ d) {
        for (int yId = _tPos.second-floor(_exactWidth/2.0)+d; yId <= _tPos.second+floor(_exactWidth/2.0)-d; ++ yId) {
            if (legal(_tPos.first+floor(_exactWidth*wRatio)+d, yId)) {
                _vPGrid.push_back(_vGrid[_tPos.first+floor(_exactWidth*wRatio)+d][yId]);
            }
            if (legal(_tPos.first-floor(_exactWidth*wRatio)-d, yId)) {
                _vPGrid.push_back(_vGrid[_tPos.first-floor(_exactWidth*wRatio)-d][yId]);
            }
    ...
    while(node->parent() != node) {
        ...
        int xId = node->parent()->xId();
        int yId = node->parent()->yId();
        int w = _exactWidth;
        if (node->parent()->xId() == node->xId()+1 && node->parent()->yId() == node->yId()) {   // Right
            for (int y = yId-floor(w*wRatio); y <= yId+floor(w*wRatio); ++ y) {
                // cerr << "x = " << xId+floor(w/2.0) << ", y = " << y << endl;
                if (legal(xId+floor(w/2.0), y)) {
                    _vPGrid.push_back(_vGrid[xId+floor(w/2.0)][y]);
                }
            }
            for (int d = 1; d <= floor(w/2.0)-floor(w*wRatio); ++ d) {
                if (legal(xId+floor(w/2.0)-d, yId+floor(w*wRatio)+d)) {
                    _vPGrid.push_back(_vGrid[xId+floor(w/2.0)-d][yId+floor(w*wRatio)+d]);
                }
                if (legal(xId+floor(w/2.0)-d, yId-floor(w*wRatio)-d)) {
                    _vPGrid.push_back(_vGrid[xId+floor(w/2.0)-d][yId-floor(w*wRatio)-d]);
                }
            ...
            for (int x = xId-floor(w*wRatio); x <= xId+floor(w*wRatio); ++ x) {
                // cerr << "x = " << x << ", y = " << yId+floor(w/2.0) << endl;
                if (legal(x, yId+floor(w/2.0))) {
                    _vPGrid.push_back(_vGrid[x][yId+floor(w/2.0)]);
                }
            }
            for (int d = 1; d <= floor(w/2.0)-floor(w*wRatio); ++ d) {
                if (legal(xId+floor(w*wRatio)+d, yId+floor(w/2.0)-d)) {
                    _vPGrid.push_back(_vGrid[xId+floor(w*wRatio)+d][yId+floor(w/2.0)-d]);
                }
                if (legal(xId-floor(w*wRatio)-d, yId+floor(w/2.0)-d)) {
                    _vPGrid.push_back(_vGrid[xId-floor(w*wRatio)-d][yId+floor(w/2.0)-d]);
                }
            ...
            for (int y = yId-floor(w*wRatio); y <= yId+floor(w*wRatio)-1; ++ y) {
                // cerr << "x = " << xId+floor(w/2.0) << ", y = " << y << endl;
                if (legal(xId+floor(w/2.0), y)) {
                    _vPGrid.push_back(_vGrid[xId+floor(w/2.0)][y]);
                }
            ...
            for (int d = 1; d <= floor(w/2.0)-floor(w*wRatio); ++ d) {
                if (legal(xId+floor(w*wRatio)+d-1, yId+floor(w/2.0)-d)) {
                    _vPGrid.push_back(_vGrid[xId+floor(w*wRatio)+d-1][yId+floor(w/2.0)-d]);
                }
                if (legal(xId+floor(w*wRatio)+d, yId+floor(w/2.0)-d)) {
                    _vPGrid.push_back(_vGrid[xId+floor(w*wRatio)+d][yId+floor(w/2.0)-d]);
                }
            ...
            for (int y = yId-floor(w*wRatio)+1; y <= yId+floor(w*wRatio); ++ y) {
                // cerr << "x = " << xId+floor(w/2.0) << ", y = " << y << endl;
                if (legal(xId+floor(w/2.0), y)) {
                    _vPGrid.push_back(_vGrid[xId+floor(w/2.0)][y]);
                }
...
void OctAStarRouter::backTraceNoPad() {
    ...
    if (encloseNode(_path[sPathId]->xId(), _path[sPathId]->yId(), floor(_exactWidth*wRatio), _sRealPos.first, _sRealPos.second)) {
        while(encloseNode(_path[sPathId]->xId(), _path[sPathId]->yId(), floor(_exactWidth*wRatio), _sRealPos.first, _sRealPos.second)) {
            if (sPathId == 0) {
                TEncloseS == true;
                break;
            }
        ...
        sPathId ++;
        assert(encloseNode(_path[sPathId]->xId(), _path[sPathId]->yId(), floor(_exactWidth*wRatio), _sRealPos.first, _sRealPos.second));
    }
    assert(encloseNode(_path[sPathId]->xId(), _path[sPathId]->yId(), halfWidth, _sRealPos.first, _sRealPos.second));

    assert(encloseNode(_path[tPathId]->xId(), _path[tPathId]->yId(), halfWidth, _tRealPos.first, _tRealPos.second));
    if (encloseNode(_path[tPathId]->xId(), _path[tPathId]->yId(), floor(_exactWidth*wRatio), _tRealPos.first, _tRealPos.second)) {
        while(encloseNode(_path[tPathId]->xId(), _path[tPathId]->yId(), floor(_exactWidth*wRatio), _tRealPos.first, _tRealPos.second)) {
        ...
        tPathId --;
        assert(encloseNode(_path[tPathId]->xId(), _path[tPathId]->yId(), floor(_exactWidth*wRatio), _tRealPos.first, _tRealPos.second));
    }
    assert(encloseNode(_path[tPathId]->xId(), _path[tPathId]->yId(), halfWidth, _tRealPos.first, _tRealPos.second));
    ...
    if (TEncloseS) {
        ...
        // around the ending grid
        // (middle part)
        for (int xId = _path[tPathId]->xId() - floor(_exactWidth*wRatio); xId <= _path[tPathId]->xId() + floor(_exactWidth*wRatio); ++ xId) {
            for (int yId = _path[tPathId]->yId() - floor(_exactWidth/2.0); yId <= _path[tPathId]->yId() + floor(_exactWidth/2.0); ++ yId) {
                if (legal(xId, yId)) {
                    _vPGrid.push_back(_vGrid[xId][yId]);
                }
            }
        ...
        // (left and right parts)
        for (int d = 1; d <= floor(_exactWidth/2.0)-floor(_exactWidth*wRatio); ++ d) {
            for (int yId = _path[tPathId]->yId()-floor(_exactWidth/2.0)+d; yId <= _path[tPathId]->yId()+floor(_exactWidth/2.0)-d; ++ yId) {
                if (legal(_path[tPathId]->xId()+floor(_exactWidth*wRatio)+d, yId)) {
                    _vPGrid.push_back(_vGrid[_path[tPathId]->xId()+floor(_exactWidth*wRatio)+d][yId]);
                }
                if (legal(_path[tPathId]->xId()-floor(_exactWidth*wRatio)-d, yId)) {
                    _vPGrid.push_back(_vGrid[_path[tPathId]->xId()-floor(_exactWidth*wRatio)-d][yId]);
                }
        ...
        for (size_t pathId = tPathId; pathId < sPathId; ++ pathId) {
            ...
            // vector<Grid*> gg;
            // if (node->parent()->xId() == 21 && node->parent()->yId() ==  58) cerr << gg[2] << endl;
            int xId = _path[pathId+1]->xId();
            int yId = _path[pathId+1]->yId();
            int w = _exactWidth;
...
double OctAStarRouter::marginCongestCost(int xId, int yId, Direction dir) {
    ...
    for (int w = ceil(_lbWidth/_gridWidth); prob(w) > threshold; ++ w) {
        ...
        if (dir == Direction::Up) {
            ...
            for (int d = 1; d <= floor(w/2.0)-floor(w*wRatio); ++ d) {
                if (legal(xId+floor(w*wRatio)+d, yId+floor(w/2.0)-d)) {
                    wCongestion += _vGrid[xId+floor(w*wRatio)+d][yId+floor(w/2.0)-d]->congestion();
                } else {
                    wCongestion += _obsCongest;
                }
                if (legal(xId-floor(w*wRatio)-d, yId+floor(w/2.0)-d)) {
            ...
            for (int d = 1; d <= floor(w/2.0)-floor(w*wRatio); ++ d) {
                if (legal(xId+floor(w/2.0)-d, yId+floor(w*wRatio)+d)) {
                    wCongestion += _vGrid[xId+floor(w/2.0)-d][yId+floor(w*wRatio)+d]->congestion();
                } else {
                    wCongestion += _obsCongest;
                }
                if (legal(xId+floor(w/2.0)-d, yId-floor(w*wRatio)-d)) {
            ...
            for (int d = 1; d <= floor(w/2.0)-floor(w*wRatio); ++ d) {
                if (legal(xId+floor(w*wRatio)+d-1, yId+floor(w/2.0)-d)) {
                    wCongestion += _vGrid[xId+floor(w*wRatio)+d-1][yId+floor(w/2.0)-d]->congestion();
                } else {
                    wCongestion += _obsCongest;
                }
                if (legal(xId+floor(w*wRatio)+d, yId+floor(w/2.0)-d)) {
...
Direction OctAStarRouter::giveDirection (GNode* curNode){
...
double OctAStarRouter::pathLength(int threshold, int method) {
